...
class Utils 
{
   ...
   protected static $local_params = array();
    ...
    public static function getGenericPagination($current, $total, $bypage, $listLength=11, $showLast = true, $use_icon = false)
    {
        ...
        if ($current > 1) {
            $out[] = array('id' => $current - 1, 'label' =>  (empty($use_icon) ? '´ ' : '') . dgettext('global', 'Page prÈcÈdente'), 'class' => 'prev' . (!empty($use_icon) ? ' prev-icon' : ''), 'accesskey' => 'a');
        }
        ...
        if ($current < $total) {
            $out[] = array('id' => $current + 1, 'label' => dgettext('global', 'Page suivante') . (empty($use_icon) ? ' ª' : ''), 'class' => 'next' . (!empty($use_icon) ? ' next-icon' : ''), 'accesskey' => 'z');
        }
